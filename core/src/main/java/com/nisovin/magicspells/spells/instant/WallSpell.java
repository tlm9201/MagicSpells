...
public class WallSpell extends InstantSpell implements TargetedLocationSpell {
	...
	private void makeWall(LivingEntity livingEntity, Location location, Vector direction, float power, String[] args) {
		...
		if (checkPlugins && livingEntity instanceof Player) {
			...
			MagicSpellsBlockPlaceEvent event = new MagicSpellsBlockPlaceEvent(target, eventBlockState, target, livingEntity.getEquipment().getItemInMainHand(), (Player) livingEntity, true);
